...
internet = tk.Checkbutton(root,text="Chrome 같은 것으로 인터넷 연결되죠?")
internet.place(x=30,y=90)
chatgpt = tk.Checkbutton(root,text="휴대폰으로 ChatGpt 같은 AI 사용할 수 있죠?")
...
def enter_cave():
    ...
    tool_text = tk.Label(root,text="이 동굴과 만나려면 인터넷 연결이 필요해요. \n\n" 
    "그럼 인터넷에서 만나려면 어떤 도구가 필요할까요?\n\n"
    "그게 바로 Chrome 같은 브라우저입니다.\n\n"
    "여기서도 잘 연결되나 한 번 실행해 볼까요?",
    font=("Arial", 12),
    justify="left")
    tool_text.place(x=30,y=30)
ready=tk.Button(root,text="준비 다 됐으면 우리 들어갈까요?",command=enter_cave)
