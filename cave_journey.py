...
def show_first_tools():
    ...
    "'터미널'에서 반복 실행하면서 많이 익숙해 졌어요.",y=130)
    add_previous_button("이전",show_look_back)
    add_next_button("자신감이 생겼어요.",show_if_agent_image)
#-------------------------------------------------------------------------
...
def show_water_path():
    ...
    add_next_button("행복감을 함께 느꼈어요.",show_agent_memory)
#-----------------------------------
# 8: 'Agent' 만들기 회고
#----------------------------------
def show_agent_memory():
    ...
    add_message("먼저, 일을 할 수 있도록 'LLM'연결을 했던 'app.py'를 불렀죠.\n\n"
    "이 파일이 우리 '탐사기록'을 읽고,\n"
...
def show_python_agent_image():
    clear_screen()
    image=Image.open("step9_agent.png")
    image=image.resize((900,600))
    photo=ImageTk.PhotoImage(image)
    label=tk.Label(root,image=photo)
    label.image=photo
...
def show_keep_recording():
    ...
    "간략하게 요약해서 기록하는 습관을 꼭 만드세요.\n\n"
    "아마도 그러한 좋은 습관들이,\n"
    "미래에 쉽게 적응하며 아이의 꿈도 실현시켜주는,\n"
    "나침반같은 새로운 도구가 되지 않을까요?\n\n"
    "그래서, 습관적으로 가능한 많이 기록할수록 더 좋아요.",y=130)
    add_previous_button("이전",show_two_records,y=520)
    add_next_button("꼭 기록 계속할게요.",show_share_record,y=520)
#--------------------------------
# 13: 나누고 싶은 것
#--------------------------------
def show_share_record():
    ...
    add_next_button("다같이 함께 했으면 좋겠어요!",show_final,x=600,y=550)
#--------------------------------------------------
# 14: 마무리
#-------------------------------------------------
def show_final():
    clear_screen()
    add_title("미래로 통하는 '동굴탐사'는 무엇을 의미할까요?",size=27)
    add_message("동굴이 마치 현재 디지털화된 세계의 'AI'같이 느껴지지 않나요?\n\n"
    "동굴 내부는 'AI'에 둘러 쌓여있는 우리 삶의 모습같이 느껴지지 않나요?\n\n"
    "배를 타고 흥미롭게 물길로 동굴을 쉽게 통과하는 것은,\n"
    "지금처럼 'AI'를 직접 경험하고 한걸음 더 빨리 미래로 가는 것 아닐까요?\n\n"
    "그냥 지식으로 알고 있는 것과 작지만 직접 만들어 가면서,\n"
    "경험하고 느끼는 것은 완전히 다르다는 것을 알게 될겁니다,\n\n"
    "더 중요한 것은 '동굴탐사'처럼 우리의 의지에 따라 'AI'가\n"
    "우리의 '동반자'처럼 항상 함께할 수 있다는 겁니다.\n\n"
    "여러분들의 미래에 많은 도구들을 활용한 'AI 에이전트'과 함께,\n"
    "희망하는 밝은 미래를 마음 것 펼쳐보세요!",size=17,y=110)
    add_previous_button("이전",show_share_record,y=540)
