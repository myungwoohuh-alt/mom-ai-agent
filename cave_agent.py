...
def show_first_step():
    clear_screen()
    add_title("나가기 전에 할일이 있어요.",size=27)
    add_message("엄마와 함께 탐사하는 동안,\n"
    "각각 자신의 느낌을 'trip.txt'에 요약 정리해 놓았죠?\n\n"
    "혹시 빠진 것들이 있을지 모르니,\n"
    "지금이라도 잊어버리기 전에 기록으로 충분히 남기세요.\n\n"
    "기록할 때 먼저 (아이)와(엄마)라고 꼭 앞에 쓰고,\n"
    "서로 다른 각각의 생각과 느낌을 요약해 보세요.\n\n"
    "기억에 남는 것, 새롭게 알게 된 것, 느꼈던 것,\n"
    "미래에 해보고 싶은 것들을 많이 기록할 수록 도움 됩니다.\n\n"
    "그리고나서 우리가 경험했던 이런 기록들을,\n"
    "정리해서 갖고 나가면 더 좋지 않을까요?",size=17,y=130)
    add_previous_button("이전",show_cave_end,y=540)
    add_next_button("좋아요!",show_new_tool,y=540)
#-----------------------------------------
...
def show_new_tool():
    ...
    "먼저, 이러한 목표를 도와줄 'Python'파일은,\n" 
    "'물길탐사'에서 'LLM'을 연결했던 'app.py'에서 진행할 겁니다.\n\n"
    "'app.py'에서 'trip.txt'의 우리 기록들을 'LLM'과 함께,\n"
    "분류 정리해 각각의 새파일에 저장까지 해 준다고 하네요.",size=18,y=130)
    add_previous_button("이전",show_first_step)
    add_next_button("'app.py'창 열었어요.",show_structure)
#--------------------------------------------
# 6:  이 구조와 의미를 알아볼까요.
#--------------------------------------------
def show_structure():
    clear_screen()
    add_title("구조와 의미를 알아볼까요?\n\n",size=28)
    add_message("동굴에게 'trip.txt'에 기록된 탐사기록을,\n"
    "각각 분류해서 계속 사용할 수 있게 해달라고 부탁했어요.\n\n"
    "그리고, 우리는 이러한 작업을 쉽게할 수 있는,\n"
    "'Python' 파일 'app.py'를 'Explorer'에서 열었습니다.\n\n"
    "이 파일은 'trip.txt.'의 기록들을 우리가 원하는대로,\n"
    "'LLM'에게 분류 정리를 부탁한 뒤, 결과를\n"
    "다시 각각의 새파일에 저장하는 일을한다고 하네요.\n\n"
...
def show_prompt_1():
    ...
    add_message("먼저 'Python' 파일 'app.py'가,\n" 
    "해야 할 일들을 간단히 살펴보아야 합니다.\n\n",size=20,y=110)
    add_message("1. 먼저 'trip.txt'에 저장된 '탐사기록'을 읽습니다.\n\n"
    "2. 'LLM'에게 기록을 분류해 정리해 달라고 부탁합니다.\n\n"
    "3. (아이)와 (엄마)의 기록을 각각 저장할 '새파일'을 만듭니다.\n\n"
    "4. '새파일'에 'LLM'이 각각 정리한 내용을 저장합니다.\n\n\n",size=15,y=220)
...
def show_prompt_2():
    clear_screen()
    add_title("어떻게 우리의 목표를 부탁하는게 효율적일까요?",size=28)
    add_message("'AI 에이전트'에게 부탁할 때는 자세하고 분명하게\n"
...
def show_code_1():
    clear_screen()
    add_title("흐름에따라 구조적으로 간결하게 코딩하세요.",size=27)
    ...
    add_message("1. 작업 시작하려면 'app.py'를 코딩으로 불러냅니다.\n" 
    "2. 그러면, '탐사기록'인 'trip.txt'를 읽고 'LLM'에게 전달 합니다.\n"
    "3. 'LLM'은 (아이)와 (엄마) 기록을 분류하고 정리합니다.\n"
    "4. 'app.py'가 두 사람의 새파일을 만들어 따로따로 저장합니다.\n"
    "5. 누군가 'VS Code'의 '터미널'에서 '실행'을 요청하면 보여줍니다.",size=14,y=290)
    add_message("복잡한 것들도 구조적으로 흐름따라 요약하면 간결하게 정리된다고합니다.",size=17,y=450)
...
def show_code_2():
    ...
    "'app.py'가 읽을 수있도록 불러오는 코딩입니다.",size=16,y=450)
    add_previous_button("이전",show_prompt_2,y=540)
    add_next_button("다음 코딩은?",show_code_3,x=700,y=540)
#-------------------------------------------
# 12: 'LLM'에게 실제로 부탁하는 코딩_1.
#------------------------------------------- 
def show_code_3():
    clear_screen()
    add_title("'LLM'에게 일을 구체적으로 부탁해 볼까요?",size=26)
    ...
    "      미래에 해보고 싶은 것:\n"
    "      \"\"\"",size=13,y=150)
    add_message("반드시 4칸 들여쓰기 (옆에 있는 'Tab'키를 사용하면 편함)와,\n"
    "'{}'들 같은 기호와 점하나까지 정확히 검토하고 입력하세요.",size=16,y=430)
    add_previous_button("이전",show_code_2,y=520)
    add_next_button("'LLM'과 연결",show_code_4,x=700,y=520)
#-------------------------------------------------
...
def show_run_agent():
    ...
    "'python app.py'라고 쓰고 직접 'Enter'로 실행하세요.\n\n"
    "우리가 코딩한 프로그램이 '탐사기록'을 다 읽고,\n"
    "'LLM'에게 어떻게 해달라고 정리를 부탁한,\n"
    "결과들이 두 사람 각각의 새 파일에 저장됩니다.\n\n"
    "왼쪽 'Explore'에 새로 만들어진,\n"
    "(아이)와 (엄마) 각각의 파일을 열어서,\n"
...
def show_agent_result_1():
    clear_screen()
    add_title("새로운 파일들이 생겼나요?",size=28)
...
def show_agent_finish():
    ...
    "그냥 아는 것과 직접 경험으로 배우는 것은 완전히 다릅니다.\n\n"
    "조금 전 우리는 처음으로\n"
    "'AI'에게 질문만 한 것이 아니라,\n"
